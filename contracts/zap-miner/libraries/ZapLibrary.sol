...
library ZapLibrary {
    ...
    function newBlock(
        ...
        for (i = 0; i < a.length; i++) {
            ...
            while (j > 0 && temp < self.currentMiners[a[j - 1]].value) {
                self.currentMiners[a[j]].value = self.currentMiners[a[j - 1]].value;
                self.currentMiners[a[j]].miner = self.currentMiners[a[j - 1]].miner;
                j--;
            }
            if (j < i) {
        ...
        _request.minersByValue[
            self.uintVars[keccak256('timeOfLastNewValue')]
        ] = [self.currentMiners[a[0]].miner, self.currentMiners[a[1]].miner, self.currentMiners[a[2]].miner, self.currentMiners[a[3]].miner, self.currentMiners[a[4]].miner];
        // _request.minersByValue[
        //     self.uintVars[keccak256('timeOfLastNewValue')]
        // ] = [a[0].miner, a[1].miner, a[2].miner, a[3].miner, a[4].miner];
        _request.valuesByTimestamp[
            self.uintVars[keccak256('timeOfLastNewValue')]
        ] = [self.currentMiners[a[0]].value, self.currentMiners[a[1]].value, self.currentMiners[a[2]].value, self.currentMiners[a[3]].value, self.currentMiners[a[4]].value];
    ...
    function submitMiningSolution(
        ...
        address miner_address = self.miners[self.uintVars[keccak256('slotProgress')]];
        // console.log("SLOT PROGRESS", self.uintVars[keccak256('slotProgress')]);
        // console.log("MINER LENGTH", self.miners.length);

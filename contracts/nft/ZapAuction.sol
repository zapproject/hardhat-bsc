...
contract ZapAuction is IERC721Receiver {
    ...
    mapping(address => mapping(uint256 => uint256)) private _allTokenAuctionsIndex;
    ...
    function tokenAuctionIdByIndex(address _nft, uint256 index) public view returns (uint256) {
        require(index < tokenAuctionSupply(_nft), "ERC721Enumerable: global index out of bounds");
    ...
    */
    function tokenAuctionByIndex(address _nft, uint256 index) public view returns (AuctionInfo memory) {
    ...
    function _addTokenToAllTokensEnumeration(address _nft, uint256 tokenId) private {
        _allTokenAuctionsIndex[_nft][tokenId] = _allTokenAuctions[_nft].length;
        _allTokenAuctions[_nft].push(tokenId);
    }

    function _removeTokenFromAllTokensEnumeration(address _nft, uint256 tokenId) private {
        ...
        uint256 lastTokenAuctionId = _allTokenAuctions[_nft][lastTokenAuctionIndex];
    ...
    */
    function bid(address _nft, uint256 _tokenId) external payable {
        ...
        require(msg.value >= auctionInfo.price, "bid price is less than current price");
        require(auctionInfo.isActive, "auction not active");
        require(auctionInfo.duration > block.timestamp, "Deadline already passed");
        ...
        if (auctionInfo.bidAmounts.length == 0) {
            ...
            require(auctionInfo.bidAmounts[lastIndex] < msg.value, "Current max bid is higher than your bid");
            auctionInfo.maxBid = msg.value;
            auctionInfo.maxBidUser = msg.sender;
        }
    ...
    */
    function executeSale(address _nft, uint256 _tokenId) external {
        AuctionInfo storage auctionInfo = tokenToAuction[_nft][_tokenId];
        require(auctionInfo.duration <= block.timestamp, "Deadline did not pass yet");
        ...
        if (auctionInfo.bidAmounts.length == 0) {
            ...
            (bool success, ) = auctionInfo.seller.call{value: auctionInfo.maxBid}("");
            require(success);
            for (uint256 i = 0; i < auctionInfo.users.length; i++) {
                address user = auctionInfo.users[i];
                if (user != auctionInfo.maxBidUser && bids[_nft][_tokenId][user] > 0) {
                    (success, ) = user.call{
                        value: bids[_nft][_tokenId][user]
                    }("");
                    require(success);
    ...
    */
    function cancelAuction(address _nft, uint256 _tokenId) external {
        ...
        ERC721(_nft).safeTransferFrom(address(this), auctionInfo.seller, _tokenId);
        _removeTokenFromAllTokensEnumeration(_nft, _tokenId);
    }

    function getTokenAuctionDetails(address _nft, uint256 _tokenId) public view returns (AuctionInfo memory) {

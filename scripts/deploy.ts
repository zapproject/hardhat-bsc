...
async function main() {
    ...
    let Zap = await zap.deploy(zapTokenBSCAddress);
    await Zap.deployed();
    ...
    const ZapMaster = await zapMaster.deploy(Zap.address, zapTokenBSCAddress);
    ...
    const Vault = await vault.deploy(zapTokenBSCAddress, ZapMaster.address);

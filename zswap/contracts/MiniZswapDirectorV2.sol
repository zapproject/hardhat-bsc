...
/// @notice The (older) ZswapDirector contract gives out a constant number of GZAP tokens per block.
/// It is the only address with minting rights for SUSHI.
/// The idea for this ZswapDirectorChef V2 (MCV2) contract is therefore to be the owner of a dummy token
/// that is deposited into the ZswapDirector V1 (MCV1) contract.
/// The allocation point for this pool on MCV1 is the total allocation point for all pools that receive double incentives.
contract MiniZswapDirectorV2 is BoringOwnable, BoringBatchable {
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 sushiReward = time.mul(gzapPerSecond).mul(pool.allocPoint) / totalAllocPoint;

...
describe("ZapMarket Test", () => {
    ...
    describe("Configure", () => {
        beforeEach(async () => {
            const zapMarketFactory = await ethers.getContractFactory(
                "ZapMarket"
            );
            zapMarket = (await upgrades.deployProxy(zapMarketFactory, {
        ...
        it("Should upgrade ZapMarket with the new getConfigStatus function and preserve the state", async () => {
            expect(await zapMarketV2.getConfigStatus(zapMedia1.address)).to.be
                .true;

            expect(await zapMarketV2.getConfigStatus(zapMedia2.address)).to.be
                .true;
        });
        ...
        it("Should emit a MediaContractCreated event on media contract deployment", async () => {
            ...
            const upgradedMarketFilter: EventFilter =
                zapMarketV2.filters.MediaContractCreated(
                    zapMedia2.address,
                    null,
                    null
                );
    ...
    describe("#setBidShares", () => {
        ...
        beforeEach(async () => {
            ...
            mint_tx1 = await zapMedia1
                .connect(signers[1])
                .mint(data, bidShares1);
            mint_tx2 = await zapMedia2
                .connect(signers[2])
                .mint(data, bidShares2);
        ...
        it("Should emit a Minted event when a token is minted", async () => {
            ...
            const upgradedMarketFilter: EventFilter =
                zapMarketV2.filters.Minted(0, zapMedia2.address);
        ...
        it("Should emit a Burned event when a token is burned", async () => {
            ...
            const upgradedMarketFilter: EventFilter =
                zapMarketV2.filters.Burned(0, zapMedia2.address);
            const upgradedEvent: Event = (
                await zapMarketV2.queryFilter(upgradedMarketFilter)
            )[0];
        ...
        it("Should set the bid shares if called by the media address", async () => {
            ...
            expect(sharesForToken1.prevOwner.value).to.be.equal(
                bidShares1.prevOwner.value
            );
            ...
            expect(sharesForToken1.owner.value).to.be.equal(
                bidShares1.owner.value
            );
            expect(upgradedShares1.owner.value).to.be.equal(
                bidShares1.owner.value
            );

            expect(sharesForToken2.prevOwner.value).to.be.equal(
            ...
            expect(sharesForToken2.owner.value).to.be.equal(
                bidShares2.owner.value
            );
            expect(upgradedShares2.owner.value).to.be.equal(
                bidShares2.owner.value
            );
        });
    ...
    describe("#setAsk", () => {
        ...
        it("Should reject if not called by the media address", async () => {
            ...
            await expect(
                zapMarketV2
                    .connect(signers[5])
                    .setAsk(zapMedia1.address, 1, ask1)
            ).to.be.revertedWith("Market: Only media contract");
            ...
            await expect(
                zapMarketV2
                    .connect(signers[5])
                    .setAsk(zapMedia2.address, 1, ask1)
            ).to.be.revertedWith("Market: Only media contract");
        });
        ...
        it("Should set the ask if called by the media address", async () => {
            ...
            const getAsk1 = await zapMarket.currentAskForToken(
                zapMedia1.address,
                0
            );
            ...
            const getAsk2 = await zapMarket.currentAskForToken(
                zapMedia2.address,
                0
            );
        ...
        it("Should emit an event if the ask is updated", async () => {
            ...
            expect(event_media1.args?.ask.amount.toNumber()).to.be.equal(
                ask1.amount
            );
            ...
            expect(event_media1.args?.ask.currency).to.be.equal(
                zapTokenBsc.address
            );
            ...
            expect(event_media2.args?.ask.amount.toNumber()).to.be.equal(
                ask2.amount
            );
            ...
            expect(event_media2.args?.ask.currency).to.be.equal(
                zapTokenBsc.address
            );
    ...
    describe("#setBid", () => {
        ...
        it("Should revert if the bid currency is 0 address", async () => {
            ...
            await zapTokenBsc
                .connect(signers[0])
                .approve(bid1.bidder, bid1.amount);
            await zapTokenBsc
                .connect(signers[0])
                .approve(bid2.bidder, bid2.amount);
        ...
        it("Should accept a valid bid larger than the min bid", async () => {
            ...
            const beforeBalance1 = await zapTokenBsc.balanceOf(
                largerBid1.bidder
            );
            const beforeBalance2 = await zapTokenBsc.balanceOf(
                largerBid2.bidder
            );
            ...
            const afterBalance1 = await zapTokenBsc.balanceOf(
                largerBid1.bidder
            );
            const afterBalance2 = await zapTokenBsc.balanceOf(
                largerBid2.bidder
            );
        ...
        it("Should emit a bid event", async () => {
            ...
            const upgraded_filter1: EventFilter =
                zapMarketV2.filters.BidCreated(zapMedia1.address, null, null);
            ...
            const upgraded_filter2: EventFilter =
                zapMarketV2.filters.BidCreated(zapMedia2.address, null, null);

            const eventLog2: Event = (await zapMarket.queryFilter(filter2))[0];
            const upgradedLog2: EventFilter = (
                await zapMarketV2.queryFilter(upgraded_filter2)
            )[0];
            ...
            expect(upgradedLog1.args?.bid.amount.toNumber()).to.equal(
                bid1.amount
            );

...
describe("ZapBondage", () => {
  ...
  it("BONDAGE_5 - calcZapForDots() - Check zap for dots calculating", async function () {
    ...
    await registry.connect(subscriber).initiateProviderCurve(specifier, piecewiseFunction, broker.address);
    ...
    const res0 = await bondage.calcZapForDots(subscriber.address, specifier, totalBound);
    console.log(res0)
...
it("BONDAGE_6 - calcZapForDots() - Check that function revert if curve not intialized", async function () {
  ...
  await expect(bondage.calcZapForDots(oracle.address, specifier, 5)).to.reverted
})
it("BONDAGE_7 - unbond() - Check unbond zap for dots calculation", async function () {
  ...
  expect(final_balance.sub(bond_balance)).to.equal(ethers.BigNumber.from(100));
});
it("BONDAGE_8 - getBoundDots() - Check received dots getting", async function () {
  ...
  const res = await bondage.connect(subscriber).getBoundDots(subscriber.address, oracle.address, specifier);
...
it("BONDAGE_10 - getZapBound() - Check received ZAP getting", async function () {
  ...
  const res = await bondage.connect(subscriber).getZapBound(oracle.address, specifier);
...
it("BONDAGE_12 - escrowDots() - Check that operator can escrow dots", async function () {
  ...
  await bondage.connect(arbiter).escrowDots(subscriber.address, oracle.address, specifier, dotsForEscrow);

  const subscriberDots = await bondage.getBoundDots(subscriber.address, oracle.address, specifier);
  expect(subscriberDots).to.equal(ethers.BigNumber.from(dots - dotsForEscrow));

  const escrowDots = await bondage.getNumEscrow(subscriber.address, oracle.address, specifier);
...
it("BONDAGE_13 - escrowDots() - Check that not operator can't escrow dots", async function () {
  ...
  await expect(bondage.escrowDots(subscriber.address, oracle.address, specifier, dotsForEscrow)).to.reverted
});
...
it("BONDAGE_15 - releaseDots() - Check that operator can release dots", async function () {
  ...
  await bondage.connect(arbiter).escrowDots(subscriber.address, oracle.address, specifier, dotsForEscrow);
  await bondage.connect(arbiter).releaseDots(subscriber.address, oracle.address, specifier, dotsForEscrow);

  const subscriberDots = await bondage.getBoundDots(subscriber.address, oracle.address, specifier);
 
  expect(subscriberDots).to.equal(ethers.BigNumber.from((dots - dotsForEscrow)));
  const pendingDots = await bondage.getNumEscrow(subscriber.address, oracle.address, specifier);
 
  expect(pendingDots).to.equal(ethers.BigNumber.from(0))
  const releaseDots = await bondage.getBoundDots(oracle.address, oracle.address, specifier);
...
it("BONDAGE_16 - releaseDots() - Check that operator can't release dots if trying to release more dots than escrowed", async function () {
  ...
  await bondage.connect(arbiter).escrowDots(subscriber.address, oracle.address, specifier, dotsForEscrow);
  await expect(bondage.connect(arbiter).releaseDots(subscriber.address, oracle.address, specifier, dotsForEscrow + 2)).to.reverted
});
...
it("BONDAGE_20 - delegateBond() - Check that delegate bond can be executed", async function () {
  ...
  await bondage.connect(escrower).delegateBond(subscriber.address, oracle.address, specifier, dotBound);
});
...
it("BONDAGE_21 - returnDots() - Check that dots can be returned", async function () {
  ...
  await bondage.connect(arbiter).escrowDots(subscriber.address, oracle.address, specifier, dotsForEscrow);
  let subscriberDots = await bondage.getBoundDots(subscriber.address, oracle.address, specifier);
  console.log("SubScriber Dots",subscriberDots.toString())
  await bondage.connect(arbiter).returnDots(subscriber.address, oracle.address, specifier, 1);

  subscriberDots = await bondage.getBoundDots(subscriber.address, oracle.address, specifier);
  console.log("SubScriber Dots",subscriberDots.toString())
  expect(subscriberDots).to.equal(ethers.BigNumber.from(dots - dotsForEscrow + dotsForReturn));

  const escrowDots = await bondage.getNumEscrow(subscriber.address, oracle.address, specifier);
  console.log("Escrow Dots",subscriberDots.toString())
  expect(escrowDots).to.equal(ethers.BigNumber.from(dotsForEscrow - dotsForReturn));
});
...
it("BONDAGE_22 - returnDots() - Check that more dots can't be returned than already escrowed", async function () {
  ...
  await bondage.connect(arbiter).escrowDots(subscriber.address, oracle.address, specifier, dotsForEscrow);
  await expect(bondage.returnDots(subscriber.address, oracle.address, specifier, dotsForEscrow + 1)).to.reverted;
});
...
it("BONDAGE_23 - returnDots() - Check that more dots can't be called by someone who isn't the owner", async function () {
  ...
  await bondage.connect(arbiter).escrowDots(subscriber.address, oracle.address, specifier, dotsForEscrow);
  await expect(bondage.connect(arbiter).returnDots(subscriber.address, oracle.address, specifier, dotsForEscrow + 1)).to.reverted;
});
...
it("BONDAGE_24 - bond() - Check that broker can bond to its endpoint", async function () {
  ...
  await registry.connect(oracle).initiateProvider(publicKey, title);
  await registry.connect(oracle).initiateProviderCurve(specifier, piecewiseFunction, testBroker.address);

  let savedBroker = await registry.getEndpointBroker(oracle.address, specifier);
...
it("BONDAGE_25 - bond() - Check that nonbroker cannot bond to broker endpoint", async function () {
  ...
  await registry.connect(oracle).initiateProvider(publicKey, title);
  await registry.connect(oracle).initiateProviderCurve(specifier, piecewiseFunction, oracle.address);

  let savedBroker = await registry.getEndpointBroker(oracle.address, specifier);

  await expect(bondage.connect(subscriber).bond(oracle.address, specifier, 3,)).to.reverted;
});

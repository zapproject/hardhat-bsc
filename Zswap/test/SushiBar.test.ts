...
describe("SushiBar", function () {
  ...
  beforeEach(async function () {
    ...
    this.bar = await this.SushiBar.deploy(this.gzap.address)
  ...
  it("should work with more than one participant", async function () {
    await this.gzap.approve(this.bar.address, "100")
    await this.gzap.connect(this.bob).approve(this.bar.address, "100", { from: this.bob.address })
    ...
    // SushiBar get 20 more G-Zap from an external source.
    await this.gzap.connect(this.carol).transfer(this.bar.address, "20", { from: this.carol.address })
    // Alice deposits 10 more G-Zap. She should receive 10*30/50 = 6 shares.
    ...
    expect(await this.gzap.balanceOf(this.bar.address)).to.equal("52")
    expect(await this.gzap.balanceOf(this.alice.address)).to.equal("70")
    expect(await this.gzap.balanceOf(this.bob.address)).to.equal("98")
  })
})

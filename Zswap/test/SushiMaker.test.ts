...
describe("SushiMaker", function () {
  ...
  beforeEach(async function () {
    await deploy(this, [
      ["gzap", this.ERC20Mock, ["GZAP", "GZAP", getBigNumber("10000000")]],
    ...
    await deploy(this, [["bar", this.SushiBar, [this.gzap.address]]])
    await deploy(this, [["sushiMaker", this.SushiMaker, [this.factory.address, this.bar.address, this.gzap.address, this.weth.address]]])
    await deploy(this, [["exploiter", this.SushiMakerExploitMock, [this.sushiMaker.address]]])
    await createSLP(this, "sushiEth", this.gzap, this.weth, getBigNumber(10))
    ...
    await createSLP(this, "sushiUSDC", this.gzap, this.usdc, getBigNumber(10))
    await createSLP(this, "daiUSDC", this.dai, this.usdc, getBigNumber(10))
    await createSLP(this, "daiMIC", this.dai, this.mic, getBigNumber(10))
  })
  describe("setBridge", function () {
    it("does not allow to set bridge for G-Zap", async function () {
      await expect(this.sushiMaker.setBridge(this.gzap.address, this.weth.address)).to.be.revertedWith("SushiMaker: Invalid bridge")
    })

    it("does not allow to set bridge for WETH", async function () {
      await expect(this.sushiMaker.setBridge(this.weth.address, this.gzap.address)).to.be.revertedWith("SushiMaker: Invalid bridge")
    })
    ...
    it("emits correct event on bridge", async function () {
      await expect(this.sushiMaker.setBridge(this.dai.address, this.gzap.address))
        .to.emit(this.sushiMaker, "LogBridgeSet")
        .withArgs(this.dai.address, this.gzap.address)
    })
  })
  describe("convert", function () {
    it("should convert G-Zap - ETH", async function () {
      await this.sushiEth.transfer(this.sushiMaker.address, getBigNumber(1))
      await this.sushiMaker.convert(this.gzap.address, this.weth.address)
      expect(await this.gzap.balanceOf(this.sushiMaker.address)).to.equal(0)
      expect(await this.sushiEth.balanceOf(this.sushiMaker.address)).to.equal(0)
      expect(await this.gzap.balanceOf(this.bar.address)).to.equal("1897569270781234370")
    })
    ...
    it("should convert USDC - ETH", async function () {
      ...
      expect(await this.gzap.balanceOf(this.bar.address)).to.equal("1590898251382934275")
    })
    ...
    it("should convert USDC - G-Zap", async function () {
      await this.sushiUSDC.transfer(this.sushiMaker.address, getBigNumber(1))
      await this.sushiMaker.convert(this.usdc.address, this.gzap.address)
      expect(await this.gzap.balanceOf(this.sushiMaker.address)).to.equal(0)
    ...
    it("converts MIC/USDC using more complex path", async function () {
      ...
      await this.sushiMaker.setBridge(this.usdc.address, this.gzap.address)
    ...
    it("converts DAI/MIC using two step path", async function () {
      ...
      expect(await this.gzap.balanceOf(this.bar.address)).to.equal("1200963016721363748")
    })
    ...
    it("reverts if caller is not EOA", async function () {
      await this.sushiEth.transfer(this.sushiMaker.address, getBigNumber(1))
      await expect(this.exploiter.convert(this.gzap.address, this.weth.address)).to.be.revertedWith("SushiMaker: must use EOA")
    })
    ...
    it("reverts if no path is available", async function () {
      ...
      expect(await this.gzap.balanceOf(this.bar.address)).to.equal(0)
    })
  })
  ...
  describe("convertMultiple", function () {
    it("should allow to convert multiple", async function () {
      await this.daiEth.transfer(this.sushiMaker.address, getBigNumber(1))
      await this.sushiEth.transfer(this.sushiMaker.address, getBigNumber(1))
      await this.sushiMaker.convertMultiple([this.dai.address, this.gzap.address], [this.weth.address, this.weth.address])
      expect(await this.gzap.balanceOf(this.sushiMaker.address)).to.equal(0)
      expect(await this.daiEth.balanceOf(this.sushiMaker.address)).to.equal(0)
      expect(await this.gzap.balanceOf(this.bar.address)).to.equal("3186583558687783097")
    })
  })
})

...
async function hasConfigFile(rootDirectory) {
    ...
    for (const file of files) {
        if (fs.existsSync(path_1.join(rootDirectory, file))) {
            return true;
        }
    }
    return false;
...
config_1.subtask("hardhat-solhint:run-solhint", async (_, { config }) => {
    const { processPath } = await Promise.resolve().then(() => __importStar(require("solhint/lib/index")));
    return processPath(path_1.join(config.paths.sources, "**", "*.sol"), await getSolhintConfig(config.paths.root));
});
config_1.task("check", async (_, { run }, runSuper) => {

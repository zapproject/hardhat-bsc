...
/// @notice The (older) G-Zap contract gives out a constant number of G-Zap tokens per block.
/// It is the only address with minting rights for G-Zap.
...
contract ZapDirectorV2 is BoringOwnable, BoringBatchable {
    ...
    /// `rewardDebt` The amount of G-Zap entitled to the user.
    struct UserInfo {
        uint256 amount;
        int256 rewardDebt;
    }
    ...
    /// Also known as the amount of G-Zap to distribute per block.
    struct PoolInfo {
    ...
    IZapDirector public immutable MASTER_CHEF;
    /// @notice Address of G-Zap contract.
    ...
    /// @param _MASTER_CHEF The Z-Swap ZDV1 contract address.
    /// @param _sushi The G-Zap token contract address.
    /// @param _MASTER_PID The pool ID of the dummy token on the base ZDV1 contract.
    constructor(IZapDirector _MASTER_CHEF, IERC20 _sushi, uint256 _MASTER_PID) public {
        MASTER_CHEF = _MASTER_CHEF;
        SUSHI = _sushi;
        MASTER_PID = _MASTER_PID;
    }

    /// @notice Deposits a dummy token to `ZAP_DIRECTOR` ZDV1. This is required because ZDV1 holds the minting rights for SUSHI.
    ...
    /// @notice Update the given pool's G-Zap allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    /// @notice View function to see pending G-Zap on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pending G-Zap reward for a given user.
    ...
    /// @notice Calculates and returns the `amount` of G-Zap per block.
    ...
    /// @notice Deposit LP tokens to MCV2 for G-Zap allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of G-Zap rewards.
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and G-Zap rewards.
    ...
    /// @notice Harvests SUSHI from `ZAP_DIRECTOR` ZDV1 and pool `MASTER_PID` to this ZDV2 contract.

...
contract ZSwapPair is IZSwapPair {
    using SafeMath for *;
    ...
    */
    function initialize(
        ...
        require(msg.sender == factory, 'ZSwap: FORBIDDEN');
        require(_slopeNumerator > 0 && _slopeNumerator <= SLOPE_NUMER_LIMIT, 'ZSwap: INVALID_SLOPE_NUMERATOR');
        require(_n > 0 && _n <= MAX_N, 'ZSwap: INVALID_N');
        require(_fee <= MAX_FEE, 'ZSwap: INVALID_FEE');
        formula = _formula;
        router = _router;
        ...
        supply = 0;
        reserveRatio = uint32(
            IBancorFormula(formula).MAX_WEIGHT().div(n + 1) //change the bancor formula to Zap curve forumla
        );  // (1 / (n + 1)) * MAX_WEIGHT
    }
    ...
    */
    function setPairOwner(address _nextOwner) external override {
        require(msg.sender == pairOwner, 'ZSwap: FORBIDDEN_PAIR_OWNER');
        require(_nextOwner != address(0), 'ZSwap: INVALID_OWNER');
    ...
    */
    function deposit(address to) external override lock returns (uint256 amountBaseOut) {
        require(msg.sender == router, 'ZSwap: FORBIDDEN_DEPOSIT');
        require(deposited == false, 'ZSwap: DOUBLE_DEPOSIT');
        reserveBase = IERC20(baseToken).balanceOf(address(this));
        require(reserveBase > 0, 'ZSwap: INSUFICCIENT_BASE_RESERVE');
    ...
    */
    function withdraw(address to) external override lock returns (uint256 amountBase, uint256 amountQuote) {
        require(msg.sender == router, 'ZSwap: FORBIDDEN_WITHDRAW');
        require(deposited, 'ZSwap: UNINITIALIZED');
    ...
    */
    function swap(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut, address to) external override lock {
        require(msg.sender == router, 'ZSwap: FORBIDDEN_SWAP');
        require(deposited, 'ZSwap: UNINITIALIZED_SWAP');
        require(
            (tokenIn == baseToken && tokenOut == quoteToken) || (tokenOut == baseToken && tokenIn == quoteToken),
            'ZSwap: INCORRECT_TOKENS'
        );
        require(to != baseToken && to != quoteToken, 'ZSwap: INVALID_TO');
        require(amountOut > 0 && amountIn > 0, 'ZSwap: INSUFFICIENT_IO_AMOUNT');
        ...
        require(amountIn <= surplus, 'ZSwap: INCORRECT_INPUT_AMOUNT');
        ...
        if (tokenOut == baseToken) {
            require(getBaseOut(amountInSubFees) >= amountOut, 'ZSwap: INVALID_BASE_OUTPUT');
            require(amountOut <= reserveBase, 'ZSwap: INSUFFICIENT_BASE_RESERVE');
            ...
            require(getQuoteOut(amountInSubFees) >= amountOut, 'ZSwap: INVALID_QUOTE_OUTPUT');
            require(amountOut <= reserveQuote, 'ZSwap: INSUFFICIENT_QUOTE_RESERVE');
    ...
    */
    function price() public view override returns (uint256 quotePrice) {
        require(deposited, 'ZSwap: UNINITIALIZED_BASE_PRICE');
        quotePrice = _convert(
            baseToken,
    ...
    */
    function getBaseOut(uint256 amountQuoteIn) public view override returns (uint256 amountBaseOut) {
        require(deposited, 'ZSwap: UNINITIALIZED_BASE_OUT');
    ...
    */
    function getQuoteOut(uint256 amountBaseIn) public view override returns (uint256 amountQuoteOut) {
        require(deposited, 'ZSwap: UNINITIALIZED_QUOTE_OUT');

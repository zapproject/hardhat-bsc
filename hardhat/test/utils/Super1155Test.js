...
describe("===Haredrop001===", function () {
  ...
  beforeEach(async function () {
    ...
    // keccaks for access rights
    setUriRight = await Haredrop001.SET_URI();
    setConfigureGroupRight = await Haredrop001.CONFIGURE_GROUP();
    mintRight = await Haredrop001.MINT();
    burnRight = await Haredrop001.BURN();
    setMetadataRight = await Haredrop001.SET_METADATA();
    lockUriRight = await Haredrop001.LOCK_URI();
    lockItemUriRight = await Haredrop001.LOCK_ITEM_URI();
    lockCreationRight = await Haredrop001.LOCK_CREATION();
  ...
  describe("Constructor", function () {
    it("should initialize values as expected", async function () {
      expect(await Haredrop001.owner()).to.equal(owner.address);
      expect(await Haredrop001.name()).to.equal("Haredrop001");
      expect(await Haredrop001.metadataUri()).to.equal(originalUri);
    });
    ...
    it("should deploy a new instance where deployer is the owner", async function () {
      ...
      // ensure properties of contract is proper
      expect(await Haredrop001.owner()).to.equal(deployer.address);
      expect(await Haredrop001.name()).to.equal("Haredrop001");
      expect(await Haredrop001.metadataUri()).to.equal(originalUri);
    });
  });
  ...
  describe("version", function () {
    it("should return the correct version", async function () {
      ...
      expect(await Haredrop001.version()).to.equal(1);
    });
  });

  describe("uri", function () {
    it("should return the metadataUri", async function () {
      expect(await Haredrop001.uri(1)).to.equal(originalUri);
    });
  });
  ...
  describe("setURI", function () {
    it("Reverts: no valid permit", async function () {
      await expect(
        Haredrop001.setURI("://ipfs/newuri/{id}")
      ).to.be.revertedWith("P1");
      expect(await Haredrop001.uri(1)).to.equal(originalUri);
    });

    it("Reverts: collection has been locked", async function () {
      // lock URI permanently
      await Haredrop001.connect(owner)["lockURI()"]();
      // attempt to change URI - should revert
      await expect(
        Haredrop001.connect(owner).setURI("://ipfs/newuri/{id}")
      ).to.be.revertedWith(
        "Haredrop001: the collection URI has been permanently locked"
      );
    ...
    it("should set the metadataUri when there is a valid permit", async function () {
      await Haredrop001.connect(owner).setPermit(
        deployer.address,
        UNIVERSAL,
        setUriRight,
        ethers.constants.MaxUint256
      );
      await Haredrop001.setURI("://ipfs/newuri/{id}");
      expect(await Haredrop001.uri(1)).to.equal("://ipfs/newuri/{id}");
      expect(await Haredrop001.uriLocked()).to.equal(false);
    });
  });
  ...
  describe("setApprovalForAll, isApprovedForAll", function () {
    it("Reverts: setting approval status for self", async () => {
      await expect(
        Haredrop001.connect(deployer).setApprovalForAll(deployer.address, true)
      ).to.be.revertedWith("ERC1155: setting approval status for self");
    });
  });

  describe.only("safeTransferFrom, safeBatchTransferFrom", function () {
    beforeEach(async function () {
      // Create Itemsgroup ID = 1
      await Haredrop001.connect(owner).configureGroup(itemGroupId, {
        name: "PEPSI",
        supplyType: 0,
        supplyData: 10,
        itemType: 1,
      ...
      await Haredrop001.connect(owner).configureGroup(itemGroupId2, {
        name: "COLA",
        supplyType: 0,
        supplyData: 15,
        itemType: 1,
      ...
      // Mint fungible item
      await Haredrop001.connect(owner).mintBatch(
        deployer.address,
        [shiftedItemGroupId],
        ["7"],
        DATA
    ...
    it("Reverts: transfer to Zero address", async () => {
      // try transfering to a zero address - should revert
      await expect(
        Haredrop001.connect(signer1).safeTransferFrom(
          deployer.address,
          NULL_ADDRESS,
    ...
    it("Reverts: transfer not approved", async () => {
      // try transfering as a unapproved caller - should revert
      await expect(
        Haredrop001.connect(signer1).safeTransferFrom(
          deployer.address,
          owner.address,
    ...
    it("Reverts: transfer has insufficient balance", async () => {
      ...
      await Haredrop001.connect(deployer).setApprovalForAll(
        signer1.address,
        true
      );

      // try transfering as user with insuffienct balance
      await expect(
        Haredrop001.connect(signer1).safeTransferFrom(
          deployer.address,
          owner.address,
    ...
    it("Reverts: transferAcceptanceCheck on a contract with ERC1155Receiver but response is not a selector", async () => {
      ...
      let MockERC1155Receiver2 = await ethers.getContractFactory(
        "MockERC1155Receiver2"
      );
    ...
    it("should do transferAcceptanceCheck on a contract with ERC1155Receiver and response is a selector", async () => {
      ...
      // Create a contract with IERC1155Receiver
      let MockERC1155Receiver1 = await ethers.getContractFactory(
        "MockERC1155Receiver1"
      );
      let mockERC1155Receiver1 = await MockERC1155Receiver1.deploy();
      await mockERC1155Receiver1.deployed();

      // Try transfering to a contract with reponse as a selector
      await Haredrop001.connect(signer1).safeTransferFrom(
        deployer.address,
        mockERC1155Receiver1.address,
        shiftedItemGroupId.add(1),
        "1",
        DATA
      );
      await expect(
        await Haredrop001.connect(owner).balanceOf(
          mockERC1155Receiver1.address,
          shiftedItemGroupId.add(1)
        )
      ).to.be.equal("1");
    ...
    it("Reverts: transferBatch ids and amounts length mismatch", async () => {
      await expect(
        Haredrop001.connect(signer1).safeBatchTransferFrom(
          deployer.address,
          owner.address,
          [shiftedItemGroupId.add(1), shiftedItemGroupId2.add(1)],
          ["5", "5", "5"],
          DATA
        )
    ...
    it("Reverts: transferBatch has a insufficient balance", async () => {
      // Approve the transfer
      await Haredrop001.connect(deployer).setApprovalForAll(
        signer1.address,
        true
    ...
    it("should do batchTransferAcceptanceCheck on a contract with ERC1155Receiver and response is a selector", async () => {
      ...
      // Try transfering to a contract with reponse as a selector
      await Haredrop001.connect(signer1).safeBatchTransferFrom(
        deployer.address,
        mockERC1155Receiver1.address,
        [shiftedItemGroupId.add(1)],
        ["1"],
  ...
  describe("configureGroup & balanceOf", function () {
    it("Reverts: groupId is 0", async () => {
      await expect(
        Haredrop001.connect(owner).configureGroup(0, {
          name: "KFC",
          supplyType: 0,
          supplyData: 20000,
          itemType: 0,
      ...
      ).to.be.revertedWith("Haredrop001: group ID 0 is invalid");
    });

    it("Reverts: sender does not have the right", async () => {
      await expect(
        Haredrop001.connect(signer1).configureGroup(itemGroupId, {
          name: "KFC",
          supplyType: 0,
          supplyData: 20000,
          itemType: 0,
        ...
        "Haredrop001: you don't have rights to configure group"
      );
    });

    it("Reverts: collection is locked", async function () {
      await Haredrop001.connect(owner).lock();
      ...
      await expect(
        Haredrop001.connect(deployer).configureGroup(itemGroupId, {
          name: "KFC",
          supplyType: 0,
          supplyData: 20000,
          itemType: 0,
        ...
        "Haredrop001: the collection is locked so groups cannot be created"
      );
    });
    ...
    it("should create a group with UNIVERSAL right", async () => {
      ...
      await expect(
        await (
          await Haredrop001.connect(owner).itemGroups(itemGroupId)
        ).name
      ).to.be.equal("KFC");
    });
    ...
    it("should create a group with GROUP right for specific GROUP circumstance", async () => {
      ...
      await Haredrop001.connect(deployer).configureGroup(specificGroup, {
        name: "BURGERKING",
        supplyType: 0,
        supplyData: 20000,
        itemType: 0,
      ...
      await expect(
        await (
          await Haredrop001.connect(owner).itemGroups(specificGroup)
        ).name
    ...
    it("tests the editability of a group with multiple reverts handling", async () => {
      ...
      await expect(
        await Haredrop001.balanceOf(deployer.address, shiftedItemGroupId.add(1))
      ).to.be.equal("10");
      ...
      await expect(
        ...
        "Haredrop001: you may not decrease supply below the circulating amount"
      );
      ...
      await expect(
        ...
        "Haredrop001: you may not uncap a capped supply type"
      );
      ...
      await expect(
        ...
        "Haredrop001: you may not increase the supply of a capped type"
      );
      ...
      // Burn some tokens
      await Haredrop001.connect(owner).burn(
        deployer.address,
        shiftedItemGroupId.add(1),
        "8"
      );
      ...
      await expect(
        ...
        "Haredrop001: the fungible item is not unique enough to change"
      );
      ...
      Haredrop001.connect(owner).burn(
        deployer.address,
        shiftedItemGroupId,
        "1"
      );
      ...
      ).to.be.revertedWith("Haredrop001: you may not alter nonfungible items");

      // Check for semi-fungible
      // Create Capped, Fungible, Burnable (Changes) itemType
      Haredrop001.connect(deployer).configureGroup(itemGroupId2, {
        name: "MCDONALD",
        supplyType: 0,
        supplyData: 2,
        itemType: 1,
      ...
      await expect(
        await (
          await Haredrop001.connect(owner).itemGroups(itemGroupId2)
        ).name
      ...
      await Haredrop001.connect(deployer).configureGroup(itemGroupId2, {
        name: "MCDONALDv2",
        supplyType: 0,
        supplyData: 2,
        itemType: 2,
    ...
    it("Reverts: balance of Zero address", async function () {
      await expect(Haredrop001.balanceOf(NULL_ADDRESS, 0)).to.be.revertedWith(
    ...
    it("should get the correct balance of a token relating to an address", async function () {
      await expect(
        await Haredrop001.balanceOf(deployer.address, 0)
      ).to.be.equal(0);
    });
  });
  ...
  describe("balanceOfBatch", function () {
    it("Reverts: ids and amounts length mismatch", async function () {
      ...
      await expect(
        Haredrop001.connect(owner).balanceOfBatch(
    ...
    it("should return an array of balances mapped to addresses", async function () {
      ...
      // Check if the minted balances are correct
      let balances = await Haredrop001.connect(owner).balanceOfBatch(
  ...
  describe("mintBatch, mintChecker", function () {
    beforeEach(async function () {
      ...
      await Haredrop001.connect(owner).configureGroup(itemGroupId2.add(1), {
        name: "MIRANDA",
        supplyType: 1,
        supplyData: 1,
        itemType: 0,
      ...
      await Haredrop001.connect(owner).configureGroup(itemGroupId2.add(2), {
        name: "SEVENUP",
        supplyType: 1,
        supplyData: 1,
        itemType: 2,
    ...
    it("Reverts: mint to Zero address", async function () {
      ...
      await expect(
        Haredrop001.connect(deployer).mintBatch(
          NULL_ADDRESS,
          [shiftedItemGroupId, shiftedItemGroupId2],
          ["5", "5"],
          DATA
    ...
    it("Reverts: no valid permit", async function () {
      ...
      await expect(
        ...
        "Haredrop001: you do not have the right to mint that item"
      );
    });
    ...
    it("Reverts: mint to non-existent itemsgroup", async function () {
      ...
      await expect(
        Haredrop001.connect(deployer).mintBatch(
          deployer.address,
          [nonExistentGroup, shiftedItemGroupId2],
          ["1", "1"],
          DATA
        )
      ).to.be.revertedWith(
        "Haredrop001: you cannot mint a non-existent item group"
      );
    });
    ...
    it("Reverts: minting beyond cap", async function () {
      ...
      await expect(
        ...
        "Haredrop001: you cannot mint a group beyond its cap"
      );
    });
    ...
    it("Reverts: minting more than one non-fungible item", async function () {
      ...
      await expect(
        ...
        "Haredrop001: you cannot mint more than a single nonfungible item"
      );
    });
    ...
    it("Reverts: minting more than alloted semi-fungible items", async function () {
      ...
      await expect(
        ...
        "Haredrop001: you cannot mint more than the alloted semifungible items"
      );
    });
    ...
    it("should mint tokens in batch to an address", async function () {
      ...
      await Haredrop001.connect(deployer).mintBatch(
        deployer.address,
        [shiftedItemGroupId],
        ["2"],
        DATA
      ...
      await expect(
        await await Haredrop001.connect(owner).balanceOf(
          deployer.address,
          shiftedItemGroupId.add(1).toString()
        )
      ).to.be.equal("2");
    });
    ...
    it("should mint semi fungible tokens in batch to an address", async function () {
      ...
      await Haredrop001.connect(owner).configureGroup(itemGroupId2.add(3), {
        name: "SEVENUP",
        supplyType: 1,
        supplyData: 1,
        itemType: 2,
  ...
  describe("burn, burnBatch, burnChecker", function () {
    ...
    it("Reverts: burn has no valid permit", async function () {
      await expect(
        Haredrop001.connect(deployer).burn(
          deployer.address,
          shiftedItemGroupId.add(1),
          "5"
        )
      ).to.be.revertedWith("Haredrop001: you don't have rights to burn");
    });
    ...
    it("Reverts: burn from non-existent itemgroup", async function () {
      ...
      await expect(
        ...
        "Haredrop001: you cannot burn a non-existent item group"
      );
    });
    ...
    it("Reverts: burn limit exceeded on itemgroup", async function () {
      ...
      await expect(
        ...
        "Haredrop001: you may not exceed the burn limit on this item group"
      );
    });
    ...
    it("should burn tokens individually based on UNIVERSAL, GROUP and ITEM circumstances (Modifier)", async function () {
      ...
      await Haredrop001.connect(deployer).burn(
        deployer.address,
        shiftedItemGroupId,
        "1"
      );
      let balance = await Haredrop001.connect(owner).balanceOf(
        deployer.address,
        shiftedItemGroupId.add(1)
      );
      ...
      balance = await Haredrop001.connect(owner).balanceOf(
        deployer.address,
        shiftedItemGroupId.add(1)
      );
    ...
    it("Reverts: burnBatch from Zero address", async function () {
      ...
      await expect(
        Haredrop001.connect(owner).burnBatch(
          NULL_ADDRESS,
          [shiftedItemGroupId3, shiftedItemGroupId4],
          ["5", "5"]
        )
      ).to.be.revertedWith("ERC1155: burn from the zero address");
    });
    ...
    it("Reverts: burnBatch has no valid permit", async function () {
      ...
      await expect(
        Haredrop001.connect(deployer).burnBatch(
          deployer.address,
          [shiftedItemGroupId3, shiftedItemGroupId4],
          ["5", "5"]
        )
      ).to.be.revertedWith(
        "Haredrop001: you do not have the right to burn that item"
      );
    });
    ...
    it("Reverts: burnBatch from non-existent itemgroups", async function () {
      ...
      await expect(
        Haredrop001.connect(owner).burnBatch(
          deployer.address,
          [nonExistentGroup, shiftedItemGroupId4],
          ["5", "5"]
        )
      ).to.be.revertedWith(
        "Haredrop001: you cannot burn a non-existent item group"
    ...
    it("Reverts: burnBatch a non-burnable item group", async function () {
      ...
      await expect(
        ...
        "Haredrop001: you cannot burn a non-burnable item group"
      );
    });
    ...
    it("should burnBatch tokens in batch based on UNIVERSAL, GROUP and ITEM circumstances (Function)", async function () {
      ...
      await Haredrop001.connect(owner).burnBatch(
        deployer.address,
        [shiftedItemGroupId3, shiftedItemGroupId4],
        ["1", "1"]
      );
      let balancesBatch = await Haredrop001.connect(owner).balanceOfBatch(
        [deployer.address, deployer.address],
        [shiftedItemGroupId3.add(1), shiftedItemGroupId4.add(1)]
      );
      ...
      await Haredrop001.connect(deployer).burnBatch(
        deployer.address,
        [shiftedItemGroupId3],
        ["1"]
      );
      balancesBatch = await Haredrop001.connect(owner).balanceOfBatch(
        [deployer.address, deployer.address],
        [shiftedItemGroupId3.add(1), shiftedItemGroupId4.add(1)]
      );
  ...
  describe("setMetadata", function () {
    it("should set the metadata", async function () {
      ...
      await expect(
        await (
          await Haredrop001.connect(deployer).itemGroups(itemGroupId)
        ).name
      ).to.be.equal("YOKITOKI");

      // Set metadata
      await Haredrop001.connect(deployer).setMetadata(shiftedItemGroupId, DATA);

      // LockURI using overloaded method
      await Haredrop001.connect(deployer)["lockURI(string,uint256)"](
        DATA,
        shiftedItemGroupId
      );
      ...
      await expect(
        Haredrop001.connect(deployer).setMetadata(shiftedItemGroupId, DATA)
      ).to.be.revertedWith(
        "Haredrop001: you cannot edit this metadata because it is frozen"
      );
    });
  });
  ...
  describe("lockURI", function () {
    it("Reverts: no valid permit", async function () {
      await expect(Haredrop001["lockURI()"]()).to.be.revertedWith("P1");
      expect(await Haredrop001.uri(1)).to.equal(originalUri);
    });
    ...
    it("should set the metadataUri and lock it when there is a valid permit", async function () {
      ...
      await Haredrop001.connect(deployer).setURI("://ipfs/lockeduri/{id}");
      await Haredrop001.connect(deployer)["lockURI()"]();
      expect(await Haredrop001.uri(1)).to.equal("://ipfs/lockeduri/{id}");
      expect(await Haredrop001.uriLocked()).to.equal(true);
    });
  });
  ...
  describe("lock", function () {
    it("should lock the contract", async function () {
      ...
      await expect(
        Haredrop001.connect(owner).configureGroup(itemGroupId, {
          name: "SUBWAY",
          supplyType: 0,
          supplyData: 2,
          itemType: 1,
  ...
  describe("Synergy between (BurnTypes) <==> (burnBatch, mintBatch)", function () {
    ...
    it("should test BurnType=None group. can be minted | can not be burned | can not be reminted", async () => {
      ...
      await expect(
        Haredrop001.connect(owner).mintBatch(
          deployer.address,
          [shiftedItemGroupId.add(1)],
          [1],
          ethers.utils.id("a")
    ...
    it("should test BurnType=Burnable NFT group. can be minted | can be burned | can not be reminted", async () => {
      ...
      let group = await Haredrop001.itemGroups(itemGroupId);
      ...
      // Check circulating supply, mintcount, burncount
      group = await Haredrop001.itemGroups(itemGroupId);
